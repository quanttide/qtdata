...
func TestDatasetCRUD(t *testing.T) {
	...
	t.Run("Create and Get", func(t *testing.T) {
		...
		if !ok || id == "" {
			t.Fatalf("expected server-assigned id, got %v", item["id"])
		}
		if item["created_at"] == "" {
			t.Error("expected created_at to be set by server")
		}
	...
	t.Run("Get not found", func(t *testing.T) {
		req := httptest.NewRequest("GET", base+"/nonexistent", nil)
		rec := httptest.NewRecorder()
		mux.ServeHTTP(rec, req)
...
func TestDatasetUpdateDelete(t *testing.T) {
	...
	t.Run("Update not found", func(t *testing.T) {
		req := httptest.NewRequest("PUT", base+"/nonexistent", strings.NewReader(`{"name":"x"}`))
		req.Header.Set("Content-Type", "application/json")
		rec := httptest.NewRecorder()
		mux.ServeHTTP(rec, req)
		if rec.Code != http.StatusNotFound {
...
func TestDatasetValidation(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/v1/qtdata/datasets", strings.NewReader(`{"description":"no name"}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
