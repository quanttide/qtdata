...
### 重构
- 详情页重构为 5 Tab（总览/数据/项目/商务/资产），每 Tab 独立 screen（lib/screens/tabs/）
- 信息归属修正：交付时间线归项目页（项目管理信息），商务管理阶段（调研→复盘）归商务页
- 矩阵重新定位为交付资产地图（资产页），单元格可点开资料弹窗
- 移动端适配：Responsive 断点（<640px 隐藏侧栏）、TabBar 可横滑、导出按钮图标化

### 新增
- 商务信息模型（BusinessInfo/Payment）与 seed 数据：报价（成本法/市场法/内部价）、合同、交付、结款
- 商务页四段：报价/合同/交付/结款 + 商务管理阶段步骤条
- 移动端视口测试（responsive_test），共 21 个测试
