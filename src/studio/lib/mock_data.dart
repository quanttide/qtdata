...
final mockProjects = <String, Project>{
  'USPTO受让人标准化与上市公司匹配': const Project(
    ...
    deliveryTarget: [
      DeliveryTarget(
        phase: '数据准备',
        items: [
          DeliveryItem(name: 'USPTO原始数据下载', status: ItemStatus.done, dim: '数据'),
    ...
    blueprint: Blueprint(
      steps: [
        BlueprintStep('USPTO Assignee 原始数据下载/准备：使用 USPTO Trademark Assignment Dataset 中的 Assignee 数据。'),
        ...
        BlueprintStep('多候选匹配：先精确匹配，再用 n-gram + RapidFuzz 做模糊匹配，保留多个候选 (candidate_1 至 candidate_5)。'),
    ...
    phases: [
      ProjectPhaseDetail(
        ...
        items: [
          PhaseItem(name: 'USPTO原始受让人数据', desc: '从USPTO商标转让数据集获取Assignee原始数据', type: '原始数据'),
      ...
      ProjectPhaseDetail(
        ...
        items: [
          ...
          PhaseItem(name: '多候选模糊匹配', desc: '精确匹配 + n-gram/RapidFuzz，保留candidate_1至candidate_5', type: '匹配结果'),
        ],
      ),
      ProjectPhaseDetail(
        name: '规则分流、AI审核与最终交付',
